...
class MultiFramesDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for t in range(T, T + self.opts.sample_frames):
            frame_i.append( utils.read_img(os.path.join(input_dir, video, "%06d.jpg" %(t+1)) ) )
            frame_p.append( utils.read_img(os.path.join(process_dir, video, 'neural_filter', 'output', "%06d.png" %(t+1)) ) )
